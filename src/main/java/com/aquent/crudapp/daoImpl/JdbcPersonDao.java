...
public class JdbcPersonDao implements PersonDao {
	...
	private static final String SQL_LIST_PEOPLE_BY_CLIENT_ID = "SELECT * FROM person WHERE client_id = :clientId";
	...
	public List<Person> listPeopleByClientId(Integer clientId) {
		return namedParameterJdbcTemplate.query(SQL_LIST_PEOPLE_BY_CLIENT_ID,
				new MapSqlParameterSource().addValue("clientId", clientId), new PersonRowMapper());
	}

	@Override
	@Transactional(propagation = Propagation.SUPPORTS, readOnly = true)
	public List<Person> listPeopleByNotHavingClientId(Integer clientId) {
		return namedParameterJdbcTemplate.query(SQL_READ_PEOPLE_NOT_HAVING_CLIENT_ID,
				new MapSqlParameterSource().addValue("clientId", clientId), new PersonRowMapper());
	}
